...
def add_item(path):
    ...
    with open(path, "a") as store_list:
        ...
        store_list.close()
        add_business_hours("open_hours_dinner.txt", "close_hours_dinner.txt")
        os.system('clear')
...
def del_item(path, item):
    ...
    del_business_hours("open_hours_dinner.txt", count)
    del_business_hours("close_hours_dinner.txt", count)
...
def main():
    ...
    while count == 1: 
        ...
        now_list("store_list_dinner.txt", "open_hours_dinner.txt", "close_hours_dinner.txt", Time)       
        command = input('(E)xecute program\n(L)ist all stores\n(A)dd store\n(D)elete store\n(R)eset data\nLet\'s choose what to eat ......')
      
        if command == 'L':
            os.system('clear')
            list_menu("store_list_dinner.txt")
            
        
        elif command == 'A':
            add_item("store_list_dinner.txt")
        
        elif command == 'D':
            item = input('Delete one store ......')
            del_item("store_list_dinner.txt", item)
            os.system('clear')
            print ('Data is update!\n')
            ...
            now = datetime.now()
            Time = now.hour + now.minute * 0.01
            print (random_meal("store_list_dinner.txt", "open_hours_dinner.txt", "close_hours_dinner.txt", Time))
            
        elif command == 'R':
            Insurance = input('Are you sure to reset data? (y/n)')
            if Insurance == 'y':
                reset_menu("store_list_dinner.txt", "open_hours_dinner.txt", "close_hours_dinner.txt")
            else:
                os.system('clear')
                print ('Cancel reset ......\n')
